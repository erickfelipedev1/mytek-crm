...
export async function sendMessageHandler(
  ...
  const convSelect = (comArchived: boolean) =>
    `id, organization_id, contact_id, channel, channel_session_id, is_group, group_chat_id, bot_silenced_until, provider_conversation_id, contacts:contact_id(phone_number, wa_identity, wa_lid, is_blocked), channel_sessions:channel_session_id(${CHANNEL_SESSION_REF_COLUMNS}, status${comArchived ? `, ${ARCHIVED_AT}` : ""})`;
  ...
  type Joined = {
    ...
    contact_id: string;
    channel: string;
    /** Nullable desde a 0176: webchat não tem sessão de transporte. */
  ...
  // alcança dois casos: o embed não ter trazido a sessão, e — desde a 0176 —
  ...
  if (c.channel === "webchat") {
    ...
    // sintoma que a 0176 existe para matar.
